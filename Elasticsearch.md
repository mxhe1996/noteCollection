...
大多搜索系统采用多阶段管道实现检索。管道的前期阶段比较便宜，使用快速算法定位众多潜在匹配文档。后续的阶段大多采用强有力的模型，来重排序文档。
<u>因为需要考虑到，资源密集的模型只适用于更小的预处理结果集，在平衡搜索性能和资源消耗下，而重排序可以返回更多相关的结果。</u>

### 6.1 检索阶段1

**全文检索**
es基于词频和逆文档频率排文档的顺序，其中BM25是默认的评分算法。

**向量相似度搜索**
转换数据为稀疏/稠密向量，采用KNN等相似度算法评估。

**混合技术**
将全文检索和向量相似度检索混合，采用词汇匹配（BM25）和倒数排序融合（RRF）。

RRF是一种将具有不同相关性指标的多个结果集组合成单个结果集的方法。
通常，最好的排名是组合多种排名方式实现，对每个结果集的排序分数归一化（每个排名算法的结果分布存在差异），然后线性组合，提供正确的权重，es就能提供为每个计算每个文档的综合排名分数。但是需要了解环境中每个算法的结果分布，并优化权重，这无疑增加了用户的操作难度。
而RRF算法能够提供零样本混合的排序算法。其基本概念是，结果的组合顺序由每个结果集中每个文档的位置定义，因此可以忽略排名分数。整个处理逻辑如下所示：
```python
score = 0.
for q in queries:
    if d in result(q):
        score += 1./(k + rank(result(q), d))

# queries 是一系列的查询方式
# result(q)是某一查询的结果集
# d是某一文档
# rank(result(q),d)表示 d在q查询结果集的位置，为正整数。
# k 为RRF的常量，通常是60
```
...
### 6.2.1 语义重排序
语义重排序需要复杂的机器学习模型和满足实时响应。与关键词、语义或者混合检索算法表现一致优秀。

**使用案例**：
1. 词汇(BM25)检索结果集
2. 语义检索结果集

**Cross-encoder和bi-encoder模型**
* cross-encoder将查询和文档文本拼接作为输入，计算具备查询意识的文档表达(query-aware document representations)。该方式提供更高的准确率，但需要更多的资源。当前es仅支持该模型。
* bi-encoder将查询和文本分别作为两个输入，所以他们并不意识到彼此。资源消耗较低但是准确性也较低。

**semantic中的语义重排序**
在es中，语义重排序被实现为API和检索器，详情查看[官网](https://www.elastic.co/guide/en/elasticsearch/reference/current/semantic-reranking.html#semantic-reranking-models)。
